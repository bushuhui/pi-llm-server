#!/usr/bin/env python3
"""
PI-LLM-Server 启动和 API 测试脚本

测试流程:
1. 启动 pi-llm-server 主服务
2. 等待服务启动完成
3. 测试所有 API 端点
4. 输出测试结果
"""
...
HEALTH_CHECK_TIMEOUT = 120  # 等待服务启动的最大时间 (秒)
...
class ServiceTest:
    ...
    def wait_for_service(self, max_wait=HEALTH_CHECK_TIMEOUT):
        """等待服务启动"""
        print_header("等待服务启动")
        ...
        while time.time() - start_time < max_wait:
            ...
            print(f"  等待中... ({elapsed}s/{max_wait}s)", end="\r")
            time.sleep(2)
    ...
    def run_all_tests(self):
        ...
        if status_resp:
            print(f"      响应：{json.dumps(status_resp.json(), indent=2, ensure_ascii=False)[:500]}...")
        ...
        if pdf_file.exists():
            ...
            with open(pdf_file, "rb") as f:
                ...
                data = {
                    ...
                    "response_format_zip": "false",  # 直接返回 markdown 文本
                }
                ...
                if mineru_resp:
                    # 响应可能是 ZIP 或 Markdown
                    content_type = mineru_resp.headers.get("content-type", "")
                    if "zip" in content_type:
                        print(f"      返回格式：ZIP (二进制)")
                        print(f"      响应大小：{len(mineru_resp.content)} bytes")
                    else:
                        text = mineru_resp.text
                        print(f"      返回格式：Markdown")
...
def main():
    """主函数"""
    print_header("PI-LLM-Server 启动和 API 测试")
    print(f"  服务地址：{BASE_URL}")
    print(f"  测试 Token: {TEST_TOKEN[:10]}...")
    print(f"  数据目录：{DATA_DIR}")
    print(f"  健康检查超时：{HEALTH_CHECK_TIMEOUT}s")
    print(f"  API 调用超时：{API_CHECK_TIMEOUT}s")
    ...
    try:
        ...
        if resp.status_code == 200:
            print(f"  {colored('检测到已有服务在运行!', 'yellow')}")
            print(f"  响应：{resp.json()}")
            use_existing = input("  是否使用现有服务进行测试？(y/n): ").strip().lower()
            if use_existing != 'y':
                print("  请先停止现有服务后重新运行此脚本")
                sys.exit(1)
            client.close()
    except:
        print("  没有检测到运行中的服务")
    ...
    print(f"  {colored('日志输出将显示在下方:', 'blue')}")
    ...
    # 等待服务启动
    test = ServiceTest(BASE_URL, TEST_TOKEN)
    if not test.wait_for_service():
        print("\n  服务启动失败，终止测试")
        process.terminate()
        sys.exit(1)
    ...
    try:
        ...
        print("  {colored('服务停止超时，强制终止', 'yellow')}")
        process.kill()
