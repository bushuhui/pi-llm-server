#!/usr/bin/env python3
"""
Qwen3-Reranker-0.6B vllm 服务器启动脚本
...
2. 自定义端口启动:
   python vllm_reranker_server.py --port 8083

3. 查看帮助:
   python vllm_reranker_server.py --help

4. 后台启动 (推荐):
   nohup python vllm_reranker_server.py > server.log 2>&1 &

5. 使用 GPU 启动 (多 GPU 支持):
   CUDA_VISIBLE_DEVICES=0,1 python vllm_reranker_server.py
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Qwen3-Reranker-0.6B vllm 服务器启动脚本",
...
示例用法:
  ...
  # 自定义模型路径
  python vllm_reranker_server.py --model-path /path/to/model

  # 限制 GPU 显存使用
  python vllm_reranker_server.py --gpu-memory-utilization 0.8
  ...
  nohup python vllm_reranker_server.py > server.log 2>&1 &
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="GPU 显存使用比例 (默认：0.8)"
    )
    parser.add_argument(
        "--max-model-len",
        type=int,
        default=8192,
        help="最大模型长度 (默认：8192，reranker 模型支持 32k 上下文)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--enforce-eager",
        action="store_true",
        default=True,  # 默认启用 eager 模式，避免编译问题
        help="强制使用 eager 模式，减少显存占用和编译依赖"
    )
    parser.add_argument(
        "--attention-backend",
        type=str,
        default="TRITON_ATTN",
        choices=["FLASHINFER", "TRITON_ATTN", "FLASH_ATTN", "TORCH_SDPA"],
        help="注意力后端 (默认：TRITON_ATTN，避免 nvcc 依赖)"
    )
    parser.add_argument(
        "--runner",
        type=str,
        ...
        help="运行器类型 (默认：pooling，用于 reranker/embedding 模型)"
    )
    parser.add_argument(
        "--convert",
        type=str,
        default="embed",
        choices=["auto", "classify", "embed", "mm_encoder_only", "none", "reward"],
        help="转换类型 (默认：embed，用于 reranker 模型)"
    )
    parser.add_argument(
...
def build_vllm_command(args):
    ...
    if args.convert:
        cmd.extend(["--convert", args.convert])
...
def start_server(args):
    """启动 vllm 服务器"""
    print("=" * 60)
    print("Qwen3-Reranker-0.6B vllm 服务器")
    print("=" * 60)
